...
export default function Clickulator(props: RouteComponentProps) {
  ...
  // App context state
  const [horizontalOffsetDistance, setHorizontalOffsetDistance] = useState<
    number | undefined
  >(defaults.horizontalOffsetDistance);
  const [
    horizontalOffsetDirection,
    setHorizontalOffsetDirection,
  ] = useState<string>(defaults.horizontalOffsetDirection);
  const [verticalOffsetDistance, setVerticalOffsetDistance] = useState<
    number | undefined
  >(defaults.verticalOffsetDistance);
  const [
    verticalOffsetDirection,
    setVerticalOffsetDirection,
  ] = useState<string>(defaults.verticalOffsetDirection);
  const [zeroAtDistance, setZeroAtDistance] = useState<number>(
    defaults.zeroAtDistance
  );
  const [adjustmentIncrement, setAdjustmentIncrement] = useState<number>(
  ...
  const [corrections, setCorrections] = useState<ICalculatorResult>(
